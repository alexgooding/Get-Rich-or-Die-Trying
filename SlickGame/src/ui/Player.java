	...
	public void update(int initialOffsetX, int initialOffsetY, boolean boundaryFlag, Input input, 
		...
		if(input.isKeyPressed(Input.KEY_UP)) {
			...
			for(int i=0; i<dungeonWalls.size(); i++) {
				if(this.playerPosX == dungeonWalls.get(i).getX()*16 - initialOffsetX && this.playerPosY == dungeonWalls.get(i).getY()*16- initialOffsetY) {
			...
			for(int i=0; i<roomGold.size(); i++) {
				if(this.playerPosX == roomGold.get(i).getX()*16- initialOffsetX && this.playerPosY == roomGold.get(i).getY()*16- initialOffsetY) {
		...
		if(input.isKeyPressed(Input.KEY_LEFT)) {
			...
			for(int i=0; i<roomGold.size(); i++) {
				if(this.playerPosX == roomGold.get(i).getX()*16- initialOffsetX && this.playerPosY == roomGold.get(i).getY()*16- initialOffsetY) {
					roomGold.remove(i);	//Gold disappear
					this.goldCounter++;
					this.playerScore = (this.playerScore + (1000 - (this.stepsToGold*100))); //v1.4 -- calculates the amount of steps taken for a gold piece
					this.stepsToGold = 0; //resets the steps to 0 
				}
			}
		}
